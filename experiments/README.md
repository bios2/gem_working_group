...
- **Broadcast-friendly shape contract.** Science functions take and return arrays of shape `(..., S)` with species on the last axis; leading axes (`...`) are spatial and arbitrary (empty `()` for a unit test on a single cell, `(X, Y)` for a production run). Coupling along the species axis is expressed as numpy reductions on `axis=-1` (e.g. ATN feeding, predation); coupling along spatial axes is expressed as numpy operations on the leading axes (e.g. dispersal stencils). The same code therefore runs at any dimensionality without per-cell Python loops: ATN's per-cell food-web math written on `(S,)` runs unchanged on `(X, Y, S)` through numpy broadcasting. This is a stylistic convention, not engine-enforced, but every process should follow it so that unit tests on hand-built `(S,)` arrays remain representative of production behaviour.
...
1. **A pure scientific function** with an explicit signature, e.g.
   ...
   No `grid`, no `env`, no side effects. Easy to unit-test with hand-built arrays, easy to prototype in a notebook from source, easy to reason about for a non-engineer contributor.
